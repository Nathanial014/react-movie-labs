...
export const getTvPages = async (pages = 1) => {
  const promises = [];
  for (let p = 1; p <= pages; p++) {
    promises.push(fetch(
      `https://api.themoviedb.org/3/discover/tv?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US&page=${p}`
    ).then((response) => {
      if (!response.ok) {
        return response.json().then((error) => {
          throw new Error(error.status_message || "Something went wrong");
...
export const getMovie = (args) => {
  // Support being called either as react-query queryFn (args.queryKey)
  // or directly with an id (numeric/string) from helper hooks.
  let id;
  if (args && typeof args === 'object' && args.queryKey) {
  ...
  return fetch(
    ...
    // If movie not found, resolve with null so callers can handle gracefully
...
export const getTvImages = ({ queryKey }) => {
  ...
  return fetch(
    ...
    // If TMDB returns 404 for images (no images found), return empty sets instead of throwing
    if (response.status === 404) {
      return { backdrops: [], posters: [], profiles: [] };
    }
    if (!response.ok) {
      return response.json().then((error) => { throw new Error(error.status_message || "Something went wrong"); });
    }
    return response.json();
  }).catch((error) => { throw error });
};

export const getTvVideos = ({ queryKey }) => {
  const [, idPart] = queryKey;
  const { id } = idPart;
  return fetch(
    `https://api.themoviedb.org/3/tv/${id}/videos?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US`
  ).then((response) => {
    if (!response.ok) {
      return response.json().then((error) => { throw new Error(error.status_message || "Something went wrong"); });
    }
...
export const getTvRecommendations = ({ queryKey }) => {
  const [, idPart] = queryKey;
  const { id } = idPart;
  return fetch(
    `https://api.themoviedb.org/3/tv/${id}/recommendations?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US&page=1`
  ).then((response) => {
    if (!response.ok) {
      return response.json().then((error) => { throw new Error(error.status_message || "Something went wrong"); });
    }
  ...
  export const getPersonImages = ({ queryKey }) => {
    ...
    return fetch(
      ...
      // If TMDB returns 404 for person images, return empty arrays rather than throwing
      if (response.status === 404) {
        return { profiles: [] };
      }
      if (!response.ok) {
